#!/bin/bash
# ============================================================
# Bug Hunter Recon v2.1 - DENGAN PROGRESS INDICATOR
# ============================================================
...
# PARSE ARGS
# ═══════════════════════════════════════════
MODE=""; TARGET=""; OUTDIR=""; QUICK=0; EXCLUDE=""
...
if [ "$MODE" = "single" ]; then
    ...
    if [ -n "$GITHUB_TOKEN" ]; then
        ...
        timeout 1800 github-subdomains -d "$TARGET" -t "$GITHUB_TOKEN" -o subdomains/github.txt 2>/dev/null &
        GH_PID=$!
    ...
    if [ "$MODE" = "wildcard" ] && [ $QUICK -eq 0 ]; then
        ...
        TOTAL_BASE=$(cat subdomains/*.txt 2>/dev/null | sort -u | wc -l)
        if [ $TOTAL_BASE -gt 1000 ]; then
            step_info "Target besar ($TOTAL_BASE subs) - limit permutation to top 500"
            PERM_INPUT=$(cat subdomains/*.txt 2>/dev/null | sort -u | head -500)
        else
            PERM_INPUT=$(cat subdomains/*.txt 2>/dev/null | sort -u)
        fi
        
        step_start "Permutation generator (alterx)..."
        echo "$PERM_INPUT" | alterx -silent -limit 50000 2>/dev/null > subdomains/alterx_generated.txt
...
httpx -l subdomains/all_subdomains.txt \
    ...
    -json -o live/httpx_full.json 2>/dev/null &
HTTPX_PID=$!
...
if [ $QUICK -eq 0 ] && [ $TOTAL_LIVE -lt 500 ]; then
    ...
    gowitness file -f live/live_urls.txt \
        ...
        --timeout 15 --threads 5 2>/dev/null &
    GW_PID=$!
    
    while kill -0 $GW_PID 2>/dev/null; do
        count=$(ls screenshots/ 2>/dev/null | wc -l)
        printf "\r  ${CYAN}⠋${NC} Screenshotting... ${WHITE}%d/%d${NC}     " "$count" "$TOTAL_LIVE"
        sleep 3
    done
    wait $GW_PID
    stop_spinner
    step_ok "Screenshots" "$(ls screenshots/ 2>/dev/null | wc -l)"
else
...
if [ $QUICK -eq 0 ]; then
    ...
    naabu -list subdomains/all_subdomains.txt \
        -top-ports 1000 -rate 1000 -silent \
...
GAU_PID=$!
while kill -0 $GAU_PID 2>/dev/null; do
...
if [ $TOTAL_JS -gt 0 ]; then
    ...
    # Trufflehog
    step_start "Trufflehog - verified secrets only..."
...
if command -v arjun &> /dev/null && [ $QUICK -eq 0 ]; then
    ...
    while kill -0 $ARJUN_PID 2>/dev/null; do
...
N1_PID=$!
while kill -0 $N1_PID 2>/dev/null; do
...
N2_PID=$!
while kill -0 $N2_PID 2>/dev/null; do
...
N4_PID=$!
while kill -0 $N4_PID 2>/dev/null; do
...
N5_PID=$!
while kill -0 $N5_PID 2>/dev/null; do
...
# PHASE 11: REPORTING
# ═══════════════════════════════════════════════════════════════
phase_start 11 "GENERATING FINAL REPORT" "1-2 menit"
...
# Count severity
CRITICAL=$(grep -h "\$critical\$" nuclei/*.txt 2>/dev/null | wc -l)
HIGH=$(grep -h "\$high\$" nuclei/*.txt 2>/dev/null | wc -l)
MEDIUM=$(grep -h "\$medium\$" nuclei/*.txt 2>/dev/null | wc -l)
LOW=$(grep -h "\$low\$" nuclei/*.txt 2>/dev/null | wc -l)
INFO=$(grep -h "\$info\$" nuclei/*.txt 2>/dev/null | wc -l)
...
step_start "Creating PRIORITY_FINDINGS.txt..."
{
    ...
    echo "═══════════════════════════════════════════════════"
    grep -h "\$critical\$" nuclei/*.txt 2>/dev/null || echo "  (none)"
    echo ""
    
    echo "═══════════════════════════════════════════════════"
    echo "  🟠 HIGH FINDINGS"
    echo "═══════════════════════════════════════════════════"
    grep -h "\$high\$" nuclei/*.txt 2>/dev/null || echo "  (none)"
    echo ""
    
    echo "═══════════════════════════════════════════════════"
    echo "  🎯 SUBDOMAIN TAKEOVER CANDIDATES"
    echo "═══════════════════════════════════════════════════"
    cat takeover/*.txt 2>/dev/null || echo "  (none)"
    echo ""
    
    echo "═══════════════════════════════════════════════════"
    echo "  🔐 EXPOSED SECRETS (.env, .git)"
    echo "═══════════════════════════════════════════════════"
    cat secrets/env_exposed.txt secrets/git_exposed.txt 2>/dev/null || echo "  (none)"
    echo ""
    
    echo "═══════════════════════════════════════════════════"
    echo "  🗝️  JS SECRETS (Mantra)"
    echo "═══════════════════════════════════════════════════"
    cat js/mantra_results.txt 2>/dev/null || echo "  (none)"
    echo ""
    
    echo "═══════════════════════════════════════════════════"
    echo "  ✅ VERIFIED SECRETS (Trufflehog)"
    echo "═══════════════════════════════════════════════════"
    cat js/trufflehog_verified.txt 2>/dev/null || echo "  (none)"
    echo ""
    
    echo "═══════════════════════════════════════════════════"
    echo "  📂 SENSITIVE FILES EXPOSED"
    echo "═══════════════════════════════════════════════════"
    cat secrets/exposed_files.txt 2>/dev/null || echo "  (none)"
...
cat > final/report.json << EOF
{
  "scan_info": {
    ...
    "mode": "$MODE",
    "version": "$VERSION",
    "start_time": "$(date -d @$SCRIPT_START_TIME '+%Y-%m-%d %H:%M:%S')",
  ...
  "statistics": {
    ...
    "urls": {
      "total_deduped": $(count_lines urls/all_urls_dedup.txt),
    ...
    "vuln_candidates": {
      ...
      "idor": $(count_lines params/idor_candidates.txt)
    },
    "secrets": {
      "env_exposed": $ENV_COUNT,
      "git_exposed": $GIT_COUNT,
